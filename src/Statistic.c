...
void GetRemnantDenLim(SALEcData * _sdata, Plane * _out, VTSDATAFLOAT _tol)
{
    // get the id of vacuum and density
    VtsInfo * _vsf = _sdata->VSF;
    _out->Id = 100;
    ...
    _out->Id = find_cellfield(_out->Name,_vsf);
    _out->VacuumId = find_cellfield(_out->Vacuum,_vsf);
    unsigned long localDensityId = find_cellfield("Density",_vsf);
    ...
    if(localDensityId == 100)
    {
        fprintf(stdout,"cannot find Density in cellfield\n");
        exit(0);
    }
    ...
    for(unsigned long ix=0;ix<_out->shape[0] - 1;ix++)
    {
        ...
        for(unsigned long jy=0;jy<_out->shape[1] - 1;jy++)
        {
            ...
            for(unsigned long kz=0;kz<zColumn;++kz)
            {
                double zDensity = VtmGetCellData(_sdata,localDensityId,ix,jy,kz)[0];
                double ColumnDz   = fabs(_sdata->GCLV[2][kz+1] - _sdata->GCLV[2][kz]);
                _out->data[ix][jy][0] += ColumnDx*ColumnDy*ColumnDz*VtmGetCellData(_sdata,_out->Id,ix,jy,kz)[0];
                if(zDensity < _tol && _tol>0.0)
                {
                    break;
                }
            }
